...
class PoolType {
  ...
  feeCollector?: string
  withdrawnAdmin?: string
  added?: boolean
}

export class VotingHandlerV2 implements IVotingContract {
  votingContract: any
  stakingContract: any
  poolType: PoolType = {}
  web3: any
  ...
  _completed = false
  _cancelled = false
  _tokenBAddress = ETHER_ZERO_ADDRESS
  ...
  constructor(public address: string, web3: Web3) {
    this.web3 = web3
    try {
      this.votingContract = new web3.eth.Contract(require('./abis/voting.abi.v2.json'), address)
  ...
  injectProvider() {
    ...
    this.votingContract = new web3.eth.Contract(
      ...
      process.env.VUE_APP_VOTING_V2_SOLIDITY
    )
    // this.stakingContract = new web3.eth.Contract(
    //   require('./abis/stake.abi.json'),
    //   process.env.VUE_APP_STAKE_CONTRACT_ADDRESS
  ...
  async getPoolType() {
    const poolType = await this.votingContract.methods.poolTypeInfos(process.env.VUE_APP_VOTING_POOL_TYPE).call()
    this.poolType = {
      ...poolType,
  ...
  async getPoolInfo(poolId) {
    ...
    this._tokenBAddress = (votingPoolInfo as PoolInfo).tokenB!
  ...
  async createPool(
    ...
    if (this.poolType.creationFee) {
      const bnbFee = bnHelper.toDecimalString(FixedNumber.from(missionLength).mulUnsafe(this.poolType.creationFee))
      const f = this.votingContract.methods.createPool(
        process.env.VUE_APP_VOTING_POOL_TYPE,
        missionLength,
  ...
  async fundTokenB(poolId, tokenBAddress, tokenBAmount, account) {
    const f = this.votingContract.methods.fund(poolId, tokenBAddress, tokenBAmount)
    await sendRequest(f, account)
  }
  ...
  async approved(rewardAddress, account, index = 3) {
    ...
    try {
      ...
      const allowance = await erc20Contract.methods.allowance(account, process.env.VUE_APP_ROUTER_ADDRESS).call()
  ...
  async approve(rewardAddress, account) {
    const web3 = walletStore.web3!
    const erc20Contract = new web3.eth.Contract(require('./abis/erc20.abi.json'), rewardAddress)
    const f = erc20Contract.methods.approve(process.env.VUE_APP_ROUTER_ADDRESS, web3.utils.toWei(`${2 ** 64 - 1}`))
    await sendRequest(f, account)
  }
  ...
  async isPoolFunded(poolId) {
    const [votingPoolInfo] = await blockchainHandler.etherBatchRequest(this.web3, [
      this.votingContract.methods.poolInfos(poolId),
    ])

    const tokenBAddress = (votingPoolInfo as PoolInfo).tokenB!
    return tokenBAddress !== ETHER_ZERO_ADDRESS
  }
}
