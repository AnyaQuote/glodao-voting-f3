...
export const vueFilterRegister = () => {
  ...
  Vue.filter('ddmmyyyyhhmma', (isoStr: number) => (isoStr ? moment(isoStr).format('MMM Do YYYY, hh:mm a') : ''))
  ...
  Vue.filter('MMDoYYYY', (isoStr: number) => (isoStr ? moment(isoStr).format('MMM Do[,] YYYY') : ''))
