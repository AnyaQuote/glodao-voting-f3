<template>
  <v-dialog :value="handler.showSelectDialog" max-width="500" @click:outside="handler.updateSelectDialogState(false)">
    <v-card>
      <v-card-title>
        <span>Select {{ handler.selectedSocialType }} task</span>
        <v-spacer />
        <v-icon @click="cancel">mdi-close</v-icon>
      </v-card-title>

      <v-card-text>
        <template v-if="handler.selectedSocialType === SocialType.TELEGRAM">
        ...
        <template v-if="handler.selectedSocialType === SocialType.TWITTER">
          <v-radio-group v-model="selectedTaskType">
            <v-sheet
        ...
        <template v-if="handler.selectedSocialType === SocialType.DISCORD">
          <v-radio-group v-model="selectedTaskType">
            <v-sheet
              class="py-4 px-4 mb-2 text-subtitle-2"
        ...
        <template v-if="handler.selectedSocialType === SocialType.FACEBOOK">
          <v-radio-group v-model="selectedTaskType">
            <v-sheet
              class="py-4 px-4 mb-2 text-subtitle-2"
        ...
        <template v-if="handler.selectedSocialType === SocialType.CUSTOM">
          <v-radio-group v-model="selectedTaskType">
            <v-sheet
              class="py-4 px-4 mb-2 text-subtitle-2"
...
@Observer
@Component
export default class SocialDialog extends Vue {
  ...
  readonly twitterOptions = [
    ...
    { type: SocialTaskType.LIKE, name: 'Like post' },
  ...
  cancel() {
    this.handler.updateSelectDialogState(false)
    this.selectedTaskType = EMPTY_STRING
  }

  confirm() {
    this.handler.appendSetting(this.handler.selectedSocialType, this.selectedTaskType)
    this.handler.updateSelectDialogState(false)
    this.selectedTaskType = EMPTY_STRING
  }
}
